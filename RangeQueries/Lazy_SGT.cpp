...
struct SegTree {
    ...
    void apply(int index, int start, int end, Update& u){
        if(start != end){
            lazy[index] = 1;
            updates[index].combine(u, start, end); // Assigning value of u to updates
        }
        u.apply(tree[index], start, end); // Assigning value of u to tree[index] {Range Update}
    }
...
struct Update1 {
    ...
    void apply(Node1 &a, int start, int end) { // apply update to given node
        a.val = val * (end - start + 1); // may change according to range update requirement
    }
...
void Shiddat(int Case)
{
    ...
    SegTree<Node1,Update1> g(n,v); // 0 index vector
    
    Node1 ans = g.make_query(left,right);
    g.make_update(left,right,val);
