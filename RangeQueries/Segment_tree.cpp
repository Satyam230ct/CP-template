...
struct SegTree {
	...
	{
		if (start == end) {
			u.apply(tree[index]); // Assigning val of u to tree[index]
			return;
		}
		int mid = (start + end) / 2;
		if (mid >= query_index)
...
struct Node1 {
	...
	void merge(Node1 &l, Node1 &r) { // Merge two child nodes
		val = (l.val + r.val); //%mod  // may change according to problem
	}
};
...
void solve(int Case)
{
	cin>>n; 
	vector<int> a(n);
	SegTree<Node1,Update1> g(n,a);

	Node1 ans= g.make_query(left,right);
	g.update(index,val);	// Point Update
